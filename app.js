...
async function processVideo() {
  if (!sourceFile) {
    termLog('No file selected.', 'err');
    return;
  }
  if (!ffmpeg) {
    termLog('FFmpeg engine not ready. Wait for "ENGINE READY" status.', 'err');
    if (dom.processBtn) dom.processBtn.disabled = true;
    return;
  }
  if (dom.engineLabel && dom.engineLabel.textContent !== 'ENGINE READY') {
    termLog('FFmpeg engine not ready yet. Wait for "ENGINE READY" status.', 'err');
    if (dom.processBtn) dom.processBtn.disabled = true;
    return;
  }
